...
\begin{resumen}
	This chapter will explain the motivation behind this TFG (Section \ref {cap1:sec:motivation}), the objectives that we set at the beginning of the project (Section \ref{cap1:sec:goals}) and finally , the structure of this memory is explained (Section \ref{cap1:sec:structure}).
\end{resumen}
...
Currently, despite the advance of the technologies, in many centers the boards are still created manually with paper, cardboard, cut-outs, stickers, etc. Even for situations where a board is urgently needed (on a field trip to explain the change of plans, in the supermarket when the child is throwing a tantrum, in the car to indicate that we are going to take longer than usual, \ldots) they have to do it with paper and colored pens, since they do not have any tool that allows them to create boards quickly and from any place. When having to perform this task manually, each time a board is created they have to create it from scratch. One way to speed up this task is the use of templates that allow you to have a structure each time you want to generate a board, in the template you only have to fill in the specific pictograms of each case (for example if we have a template for elections, only the pictograms would have to be modified by the different options of choice). In addition, these templates allow the boards to always have the same format and to make the message faster and more effective.
